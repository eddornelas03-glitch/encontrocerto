...
const FlagIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
    <path d="M3.5 2.75a.75.75 0 00-1.5 0v14.5a.75.75 0 001.5 0V14h5.75a.75.75 0 010 1.5H3.5v2.75a.75.75 0 001.5 0V17h5.75a.75.75 0 010 1.5H5v2.75a.75.75 0 001.5 0V20h5.75a.75.75 0 010 1.5H6.5v2.75a.75.75 0 001.5 0V18h5.75a.75.75 0 010 1.5H8V12h1.5V9.25H3.5V2.75z" clipRule="evenodd" />
...
const BlockIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
    <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" clipRule="evenodd" />
  </svg>
);
...
export const ChatWindow: React.FC<ChatWindowProps> = ({ match, onBack, currentUser, messages, onSendMessage, onSendSystemMessage, onRequestAnalysis, onViewProfile, onUnmatch }) => {
  const [isScheduling, setIsScheduling] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const [showConfirmation, setShowConfirmation] = useState<'unmatch' | 'block' | null>(null);
  ...
  return (
    ...
    <div className="h-full w-full bg-gray-800 flex flex-col">
      <header className="bg-gray-900 p-3 border-b border-gray-700 flex items-center justify-between shadow-md z-10 shrink-0">
        <div className="flex items-center">
            <button onClick={onBack} className="text-gray-300 mr-2 p-2 rounded-full hover:bg-gray-700" aria-label="Voltar para a lista de matches">
            ...
            >
                <img src={match.images[0]} alt={`Foto de perfil de ${match.name}`} className="w-10 h-10 rounded-full object-cover" />
        ...
        <div className="flex items-center gap-2">
            <button onClick={handleScheduleMeeting} className="text-pink-400 p-2 rounded-full hover:bg-pink-500/10" aria-label="Agendar Encontro">
             ...
             <div className="relative" ref={menuRef}>
                <button onClick={() => setIsMenuOpen(prev => !prev)} className="text-gray-400 p-2 rounded-full hover:bg-gray-700" aria-label="Mais opções" aria-haspopup="true" aria-expanded={isMenuOpen}>
                    <MoreIcon/>
                </button>
                {isMenuOpen && (
                    <div className="absolute top-full right-0 mt-2 w-56 bg-gray-700 rounded-lg shadow-xl z-20 py-1">
                        <ul className="text-white text-sm" role="menu">
                            <li role="menuitem">
                                <button onClick={() => { setIsReporting(true); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 text-left px-4 py-3 hover:bg-gray-600 rounded-t-lg">
                                    <FlagIcon />
                                    <span>Denunciar</span>
                                </button>
                            </li>
                             <li role="menuitem">
                                <button onClick={() => { setShowConfirmation('block'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 text-left px-4 py-3 hover:bg-gray-600">
                                    <BlockIcon />
                                    <span>Bloquear</span>
                                </button>
                            </li>
                             <li role="menuitem">
                                <button onClick={() => { setShowConfirmation('unmatch'); setIsMenuOpen(false); }} className="w-full flex items-center gap-3 text-left px-4 py-3 hover:bg-gray-600 rounded-b-lg text-red-400">
                                    <TrashIcon />
    ...
    {isReporting && <ReportModal match={match} onClose={() => setIsReporting(false)} onSubmit={handleReportSubmit} />}
