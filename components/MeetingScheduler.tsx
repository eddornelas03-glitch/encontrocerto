...
export const MeetingScheduler: React.FC<MeetingSchedulerProps> = ({ match, onClose, onSchedule }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if(date && time) {
            const formattedDate = new Date(date + 'T00:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-sm" onClick={e => e.stopPropagation()}>
                <h2 className="text-2xl font-bold text-gray-800 text-center">Agendar Encontro</h2>
                <p className="text-gray-500 text-center mt-1">Proponha um dia e horário para encontrar {match.name}.</p>
