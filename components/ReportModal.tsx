    ...
    if (isSubmitted) {
        return (
             <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4" role="alertdialog" aria-modal="true" aria-labelledby="report-submitted-title">
                <div className="bg-gray-800 rounded-2xl shadow-lg p-8 w-full max-w-sm text-center">
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="report-title">
            <div className="bg-gray-800 text-white rounded-2xl shadow-lg w-full max-w-md max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex justify-between items-center p-5 border-b border-gray-700 shrink-0">
                    <h1 id="report-title" className="text-xl font-bold text-red-400">Denunciar {match.name}</h1>
                    ...
                    <form onSubmit={handleSubmit}>
                        <fieldset>
                            ...
                            <div className="space-y-3 mb-4">
                                {reportReasons.map(reason => (
                                    <label key={reason} className="flex items-center p-3 bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-600 has-[:checked]:bg-pink-900/50 has-[:checked]:ring-2 has-[:checked]:ring-pink-500">
                                        <input
                                            type="radio"
                                            name="report-reason"
                                            value={reason}
                                            ...
                                            className="h-5 w-5 text-pink-500 bg-gray-800 border-gray-600 focus:ring-pink-600 checked:bg-pink-500"
                                        />
                        ...
                        <div className="mb-6">
                             ...
                             <textarea
                                ...
                                className="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-pink-500 text-white"
                             />
                        </div>
