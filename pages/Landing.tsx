...
const BackgroundProfileCard: React.FC<{
  ...
  <div
    className={`relative w-full aspect-[3/4] bg-gray-800 rounded-2xl shadow-xl overflow-hidden ${className}`}
  >
    <img
...
const Logo = () => (
  ...
  >
    ...
    />
    <path
      d="M10.0001 13.3L7.7001 11L6.3001 12.4L10.0001 16.1L18.0001 8.10001L16.6001 6.70001L10.0001 13.3Z"
      fill="white"
    />
  </svg>
);
        ...
        {profileColumns.map((column, colIndex) => (
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              {column.map((profile) => (
                <BackgroundProfileCard key={`${profile.id}-1`} profile={profile} />
              ))}
              {column.map((profile) => (
                <BackgroundProfileCard key={`${profile.id}-2`} profile={profile} />
              ))}
            </div>
          </div>
        ))}
        ...
        <header>
          ...
          <div className="mt-8 flex flex-col sm:flex-row justify-center gap-4">
            <button
              ...
              className="bg-pink-500 font-bold py-3 px-8 rounded-full text-lg hover:bg-pink-600 transition-colors shadow-lg"
            >
              Criar Conta
            </button>
            <button
