...
export const MatchModal: React.FC<MatchModalProps> = ({ match, currentUserImage, onClose, onSendMessage }) => {
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex justify-center items-center z-50 p-4" onClick={onClose}>
      <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl shadow-lg text-center p-6 w-full max-w-sm relative transform transition-all scale-100" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex justify-center items-center my-6 space-x-[-2rem]">
          <img src={currentUserImage} alt="Você" className="w-28 h-28 rounded-full object-cover border-4 border-pink-500 shadow-lg transform -rotate-12"/>
          <img src={match.images[0]} alt={match.name} className="w-28 h-28 rounded-full object-cover border-4 border-yellow-400 shadow-lg transform rotate-12"/>
        </div>

        <div className="bg-white/10 p-3 rounded-lg mb-6">
            <h3 className="font-semibold text-yellow-400 text-sm">Por que vocês combinam?</h3>
